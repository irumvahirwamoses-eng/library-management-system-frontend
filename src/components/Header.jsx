...
import { LogOut, User, ChevronDown, Bell, CheckCheck, Menu } from 'lucide-react';
...
export default function Header({ onMenuClick }) {
  ...
  return (
    <header className="bg-white border-b border-gray-200 px-4 sm:px-6 lg:px-8 py-3 flex items-center justify-between sticky top-0 z-30 gap-3">
      <div className="flex items-center gap-3 min-w-0">
        <button onClick={onMenuClick} className="lg:hidden p-2 -ml-1 text-gray-600 hover:bg-gray-100 rounded-xl transition" aria-label="Open menu">
          <Menu size={20} />
        </button>
      ...
      <div className="flex items-center gap-1 sm:gap-2 shrink-0">
        <div className="relative" ref={notifRef}>
          ...
          {notifOpen && (
            <div className="absolute right-0 mt-2 w-72 sm:w-80 max-w-[90vw] bg-white rounded-xl border border-gray-200 shadow-lg py-2 z-50">
        ...
        <div className="relative" ref={ref}>
          <button onClick={() => setOpen(!open)} className="flex items-center gap-2 px-2 sm:px-3 py-1.5 rounded-xl hover:bg-gray-50 transition">
